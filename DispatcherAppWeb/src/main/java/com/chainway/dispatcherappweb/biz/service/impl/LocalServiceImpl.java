package com.chainway.dispatcherappweb.biz.service.impl;
...
import com.chainway.dispatcherappweb.common.AuthUtils;
import com.chainway.dispatcherappweb.common.Constant;
...
public class LocalServiceImpl implements LocalService {
	...
	public String addUserInSession(User user) {
		...
		String userId= AuthUtils.MD5(user.getId().toString());
		//拼接uuid
		uuid=userId+uuid;
		...
		cacheService.setData(token, map,3600*24*30);//session存活30天
		return uuid;
	}
	...
	public void refreshUserInSession(String uuid) {
		...
		//再更新缓存
		cacheService.setData(token, user, -1);//-1为设置成永不过期
	}
	...
	@Override
	public void deleteUserByPreKey(User user) {
		String userId= AuthUtils.MD5(user.getId().toString());
		String preToken=Constant.USER_SESSION_KEY_PREFIX+userId;
