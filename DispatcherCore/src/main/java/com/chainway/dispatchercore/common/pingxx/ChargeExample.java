...
public class ChargeExample {
	...
	public static void runDemos(String appId) {
		...
		System.out.println("------- 查询 charge -------"+charge.getId());
//		chargeExample.retrieve("ch_y5aTqHP0ib5OS8GaPOefrbv1");
...
//		Charge charge = null;
//		String channel = "alipay_qr";
//
//		Map<String, Object> chargeMap = new HashMap<String, Object>();
//		chargeMap.put("amount", 100);// 订单总金额, 人民币单位：分（如订单总金额为 1 元，此处请填 100）
//		chargeMap.put("currency", "cny");
//		chargeMap.put("subject", "Device renew");
//		chargeMap.put("body", "Due to equipment renewal");
...
//	        extra.put("success_url", "http://127.0.0.1/succeeded");
	 ...
	 */
	public Charge createCharge(ChargeBean bean) throws AuthenticationException, InvalidRequestException,
			APIConnectionException, APIException, ChannelException, RateLimitException {
		Charge charge = null;
		String channel = bean.getChannel();

		Map<String, Object> chargeMap = new HashMap<String, Object>();
		chargeMap.put("amount", bean.getAmount());// 订单总金额, 人民币单位：分（如订单总金额为 1
													// 元，此处请填 100）
		chargeMap.put("currency", bean.getCurrency());
		chargeMap.put("subject", bean.getSubject());
		chargeMap.put("body", bean.getBody());
		String orderNo = bean.getOrderNo();
		chargeMap.put("order_no", orderNo);// 推荐使用 8-20 位，要求数字或字母，不允许其他字符
		chargeMap.put("channel", channel);// 支付使用的第三方支付渠道取值，请参考：https://www.pingxx.com/api#api-c-new
		chargeMap.put("client_ip", bean.getClientIp()); // 发起支付请求客户端的 IP 地址，格式为
														// IPV4，如:
		// 127.0.0.1
		Map<String, String> app = new HashMap<String, String>();
		app.put("id", bean.getAppId());
		chargeMap.put("app", app);

		// extra 取值请查看相应方法说明
		chargeMap.put("extra", channelExtra(channel, bean.getSuccessUrl()));
		//用户指定的 metadata 参数,每一个对象的 metadata 最多可以拥有 10 个键值对，数据总长度在 1000 个 Unicode 字符以内。
		Map<String, Object> initialMetadata = new HashMap<String, Object>();
		initialMetadata.put("user_id", bean.getUserId());
		initialMetadata.put("business_order_no",bean.getBusinessOrderNo());
	...
	private Map<String, Object> channelExtra(String channel, String url) {
		Map<String, Object> extra = new HashMap<>();
		switch (channel) {
		case "alipay_pc_direct":
			extra = alipayPcDirectExtra(url);
			break;
		case "wx_pub_qr":
			extra = wxPubQrDirectExtra();
			break;
		case "upacp_pc":
			extra = upacpPcExtra(url);
			break;
		}
	...
	private Map<String, Object> alipayPcDirectExtra() {
		Map<String, Object> extra = new HashMap<>();
		// 必须，支付成功的回调地址，在本地测试不要写 localhost ，请写 127.0.0.1。URL 后面不要加自定义参数。
		extra.put("success_url", "http://127.0.0.1:8080/ivc/pages/admin.jsp");
	...
	private Map<String, Object> alipayPcDirectExtra(String url) {
		Map<String, Object> extra = new HashMap<>();
		// 必须，支付成功的回调地址，在本地测试不要写 localhost ，请写 127.0.0.1。URL 后面不要加自定义参数。
	...
	private Map<String, Object> upacpPcExtra() {
		Map<String, Object> extra = new HashMap<>();
		// 必须，支付完成的回调地址，在本地测试不要写 localhost ，请写 127.0.0.1。URL 后面不要加自定义参数。
		extra.put("result_url", "http://127.0.0.1:8080/ivc/pages/admin.jsp");

		return extra;
	}

	private Map<String, Object> upacpPcExtra(String url) {
		Map<String, Object> extra = new HashMap<>();
		// 必须，支付完成的回调地址，在本地测试不要写 localhost ，请写 127.0.0.1。URL 后面不要加自定义参数。
		extra.put("result_url", url);
